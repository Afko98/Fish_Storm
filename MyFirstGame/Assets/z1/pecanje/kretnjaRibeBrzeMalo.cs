...
public class kretnjaRibeBrzeMalo : MonoBehaviour
{
    ...
    private void Start()
    {
        if (Camera.main.transform.position.y < 2.1f)
            if (transform.rotation.y == 0f)
                GetComponent<Rigidbody2D>().velocity = new Vector3(0.65f, 0f, 0f);
            else
                GetComponent<Rigidbody2D>().velocity = new Vector3(-0.65f, 0f, 0f);
    }
    ...
    void Update()
    {
        ...
        if (transform.tag != "uhvacena")
        {
            ...
            if (transform.position.x < -3f && transform.position.x > -5f)
            {
                ...
                GetComponent<Rigidbody2D>().velocity = new Vector3(0.65f, GetComponent<Rigidbody2D>().velocity.y, 0f);
            }
